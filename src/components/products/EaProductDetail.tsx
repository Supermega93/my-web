      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-28 space-y-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          >
            ...
            <div className="pt-2 p-5 rounded-2xl bg-[#0B101A] border border-slate-800 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  ...
                  <div className="text-3xl font-black text-white font-mono flex items-baseline gap-2">
                    ...
                    <span className="text-xs font-normal text-slate-400 font-sans">
                      {selectedTier.currency} • {selectedTier.license}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  <Button
                    ...
                    className="font-bold shadow-lg shadow-emerald-500/20 bg-emerald-500 hover:bg-emerald-400 text-slate-950"
                  >
          ...
          {/* Product Media Display: Exact 200x200 Badge with 12px border radius and seamless dark theme blending */}
          <motion.div 
          ...
          >
            <div className="relative p-8 rounded-3xl bg-gradient-to-b from-[#0F1626] to-[#070B14] border border-slate-800/90 shadow-2xl flex flex-col items-center justify-center w-full max-w-sm group">
              {/* Soft glow behind the badge */}
              <div className="absolute -inset-2 bg-emerald-500/15 rounded-3xl blur-2xl -z-10 group-hover:bg-emerald-500/25 transition-all" />

              {/* Exact 200x200 Square Badge with 8px rounded corners (rounded-lg) */}
              <div className="w-[200px] h-[200px] rounded-lg overflow-hidden border border-slate-700/80 shadow-[0_20px_45px_rgba(0,0,0,0.9)] bg-[#070B14] flex items-center justify-center">
                <img
                  src={product.image_url || STOREFRONT_MEDIA.flagshipEa.imageUrl}
                  alt={product.name}
                  referrerPolicy="no-referrer"
                  className="w-[200px] h-[200px] aspect-square object-contain rounded-lg group-hover:scale-105 transition-transform duration-500"
                />
              </div>

              <div className="w-full mt-6 p-3.5 bg-slate-950/90 rounded-xl border border-slate-800/80 flex items-center justify-between text-xs font-mono">
                <span className="text-emerald-400 font-semibold flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                  STABLE V1.0 BUILD
                </span>
                <span className="text-slate-400">MT5 (PC/Windows)</span>
              </div>
            </div>
          </motion.div>
        </div>
        ...
        <div className="pt-12 border-t border-slate-800/80 space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="p-6 rounded-2xl bg-[#0D1421] border border-slate-800 space-y-2">
              ...
              <span className="text-xs font-mono text-emerald-400 uppercase font-semibold">Platform</span>
              <h4 className="text-base font-bold text-white">MetaTrader 5 (MT5)</h4>
              <p className="text-xs text-slate-400">Engineered natively in MQL5 with asynchronous order execution.</p>
            </div>
            ...
            <div className="p-6 rounded-2xl bg-[#0D1421] border border-slate-800 space-y-2">
              ...
              <span className="text-xs font-mono text-cyan-400 uppercase font-semibold">Operating System</span>
              <h4 className="text-base font-bold text-white">Windows / PC</h4>
              <p className="text-xs text-slate-400">Compatible with Windows desktop, laptop, and 24/5 cloud VPS terminals.</p>
            </div>
            ...
            <div className="p-6 rounded-2xl bg-[#0D1421] border border-slate-800 space-y-2">
              ...
              <span className="text-xs font-mono text-teal-400 uppercase font-semibold">Instruments</span>
              <h4 className="text-base font-bold text-white">Forex & XAUUSD</h4>
              <p className="text-xs text-slate-400">Targeting high-volume major currency pairs and Gold with optimized presets.</p>
            </div>
            ...
            <div className="p-6 rounded-2xl bg-[#0D1421] border border-slate-800 space-y-2">
              ...
              <span className="text-xs font-mono text-indigo-400 uppercase font-semibold">Adaptability</span>
              <h4 className="text-base font-bold text-white">Configurable Presets</h4>
              <p className="text-xs text-slate-400">Flexible parameter inputs adaptable for different symbols and market conditions.</p>
            </div>
          </div>
        </div>

        {/* 3. CURRENT PRICING SECTION (Transparent Tier Comparison) */}
        <div ref={pricingRef} className="pt-12 border-t border-slate-800/80 space-y-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div className="space-y-2">
              ...
              <h2 className="text-2xl sm:text-3xl font-bold text-white tracking-tight">
                Current Pricing & Versions
              </h2>
            </div>
            <p className="text-xs text-slate-400 max-w-md">
              One-time investment. No recurring monthly fees. Choose the tier that matches your trading volume and infrastructure requirements.
            </p>
          </div>
            ...
            {pricingTiers.map((tier) => {
              ...
              return (
                <div
                  ...
                  className={`p-8 rounded-3xl transition-all duration-300 relative flex flex-col justify-between cursor-pointer ${
                    isSelected
                      ? 'bg-[#0E1726] border-2 border-emerald-500 shadow-2xl shadow-emerald-500/10'
                      : 'bg-[#0B101A] border border-slate-800 hover:border-slate-700'
                  }`}
                >
                  {tier.badge && (
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-emerald-500 text-slate-950 font-mono text-[10px] font-bold uppercase tracking-wider">
                      {tier.badge}
                    </div>
                  )}
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="text-xs text-emerald-400 font-mono mt-1">{tier.license}</div>
                    </div>
                    ...
                    <div className="pt-2">
                      ...
                      <div className="text-xs text-slate-400 font-mono mt-1">
                        {tier.currency} • Lifetime License
                      </div>
                    </div>
        ...
        <div ref={detailsRef} className="pt-12 border-t border-slate-800/80 space-y-8">
          <div className="space-y-2">
            <span className="text-xs font-mono uppercase tracking-widest text-emerald-400 font-semibold">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {coreFeatures.map((feat, idx) => (
              <div
                key={idx}
                className="p-6 rounded-2xl bg-[#0B101A] border border-slate-800 hover:border-emerald-500/30 transition-all flex flex-col justify-between space-y-2.5"
              >
        ...
        <div className="pt-12 border-t border-slate-800/80 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {marketplaceFeatures.map((item, idx) => (
              <div
                key={idx}
                className="p-6 rounded-2xl bg-[#0D1421] border border-slate-800 space-y-2"
              >
              ...
              return (
                ...
                >
                  <button
                    ...
                    className="w-full p-4 text-left flex items-center justify-between text-sm font-semibold text-white hover:text-emerald-400 transition-colors"
                  >
                    <span>{faq.q}</span>
                    <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${isOpen ? 'rotate-180 text-emerald-400' : 'text-slate-400'}`} />
                  </button>
                  {isOpen && (
        ...
        <div className="p-6 rounded-2xl bg-slate-950/80 border border-slate-800 text-slate-400 text-xs space-y-2 leading-relaxed">
        ...
        {/* 8. BOTTOM BUY BAR */}
        <div className="p-8 rounded-3xl bg-gradient-to-r from-slate-900 via-[#101929] to-slate-900 border border-emerald-500/30 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-2xl">
          <div>
            <div className="text-2xl font-bold text-white">Adaptive Liquidity Pro V1.0</div>
            <div className="text-xs text-slate-400">
              Selected: <span className="text-emerald-400 font-semibold">{selectedTier.name} ({selectedTier.displayPrice})</span> • Instant download with centralized license provisioning
            </div>
          </div>
          ...
          <Button
            ...
            onClick={() => handlePurchase(selectedTier)}
            icon={<ArrowRight className="w-4 h-4" />}
            className="font-bold shadow-lg shadow-emerald-500/25 bg-emerald-500 hover:bg-emerald-400 text-slate-950 shrink-0"
          >
